...
namespace AxoSynaptic.Gui
{
  public class Controller : IController, Object
  {
    ...
    protected void search_delete_word ()
    {
      ...
      if (needle == "") return;
      model.query[mode] = AxoSynaptic.Utils.remove_last_word (needle);
    ...
    protected void search_add_delete_char (string? newchar = null)
    {
      ...
      if (newchar == null)
      {
        // delete
        if (needle == "") return;
        model.query[mode] = AxoSynaptic.Utils.remove_last_unichar (needle);
      }
      else
      {
        // add
      ...
      if (command != KeyComboConfig.Commands.INVALID_COMMAND)
      {
        switch (command)
        {
          ...
          case KeyComboConfig.Commands.ACTIVATE:
            ...
            view.set_list_visible (false);
            break;
          case KeyComboConfig.Commands.EXIT_AXOSYNAPTIC:
            Gtk.main_quit ();
            break;
