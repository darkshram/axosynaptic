...
namespace AxoSynaptic
{
  public class UILauncher : GLib.Object
  {
    ...
    private void init_indicator ()
    {
      ...
      // https://bugs.launchpad.net/axosynaptic-project/+bug/685634/comments/13
      indicator = new AppIndicator.Indicator ("axosynaptic", "axosynaptic",
    ...
    {
      /* try to fix IBUS input method adding axosynaptic to no-snooper-apps */
      string ibus_no_snooper = GLib.Environment.get_variable ("IBUS_NO_SNOOPER_APPS");
      if (ibus_no_snooper == null || ibus_no_snooper == "")
      {
        GLib.Environment.set_variable ("IBUS_NO_SNOOPER_APPS", "axosynaptic", true);
        return;
      }
      if ("axosynaptic" in ibus_no_snooper) return;
      /* add axosynaptic */
      if (!ibus_no_snooper.has_suffix (","))
        ibus_no_snooper = ibus_no_snooper + ",";
      ibus_no_snooper = ibus_no_snooper + "axosynaptic";
    ...
    public static int main (string[] argv)
    {
      ...
      var context = new OptionContext (" - AxoSynaptic");
      ...
      try
      {
        ...
        load_custom_style ();
        Gtk.init (ref argv);
        Notify.init ("axosynaptic");

        var app = new GLib.Application ("org.mate.axosynaptic", ApplicationFlags.FLAGS_NONE);
        if (!app.register () || app.get_is_remote ()) {
          message ("AxoSynaptic is already running, activating...");
          app.activate ();
        }
        else
        {
